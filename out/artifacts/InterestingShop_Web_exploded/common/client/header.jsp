  ...
  <div class="sou">
    <!--Begin 所在收货地区 Begin-->
        ...
        <span class="fr">
          ...
          <c:if test="${sessionScope.loginUser!=null}">
            <span class="fl"><a href="${ctx}/manager/user?action=index">${sessionScope.loginUser.nickName}</a>&nbsp;|&nbsp;<a href="${ctx}/manager/order?action=index&userId=${sessionScope.loginUser.id}">我的订单</a>&nbsp;</span>
          </c:if>
           <c:if test="${sessionScope.loginUser!=null && sessionScope.loginUser.type==1}">
            <span class="fl">|&nbsp;<a href="${ctx}/manager/goods?action=index&userId=${sessionScope.loginUser.id}">后台管理</a>&nbsp;</span>
          </c:if>
           <c:if test="${sessionScope.loginUser!=null}">
